...
/*
 * add geoipisp-csv-to-dat
 * by yubo@xiaomi.com
 */

#define MAXSEGMENTS 0xF0000000
#define MAXBUFF 10*1024*1024
...
void error (int __status, int __errnum, const char *__format, ...)
{
    ...
    fprintf(stderr, "status [%d] errnum [%d] ", __status, __errnum);
                    ...
                    unsigned int __lineno, const char *__format, ...)
{
    ...
    va_start(args, __format);
    fprintf(stderr, "status[%d] errnum[%d] ", __status, __errnum);
    fprintf(stderr, "fname[%s] errnum[%u] ", __fname, __lineno);
    vfprintf(stderr, __format, args);
    fprintf(stderr, "\n");
    fflush(stderr);
    va_end(args);
...
char program_invocation_name[] = "geoip-generator";
...
namespace {
	...
	public:
		...
		void write_binary(std::ostream &dat_stream, struct isp *isp) const;
	...
	const std::vector<edge_type> &new_to_old)
{
	...
	for (std::vector<edge_type>::const_iterator it = new_to_old.begin();
...
void
binary_trie::write_binary(std::ostream &dat_stream, struct isp *isp) const
{
	edge_type a, b;
	for (std::vector<node>::const_iterator it = nodes.begin();
		...
		if(a > MAXSEGMENTS){
			a = a - MAXSEGMENTS + nodes.size();
		}
		if(b > MAXSEGMENTS){
			b = b - MAXSEGMENTS + nodes.size();
		}
		...
		dat_stream.write(binary.chars, 8);
		if (dat_stream.bad())
			return;
	}
	
	dat_stream.write(isp->name, isp->len);
	
	isp->segments[0] = static_cast<uint8_t>((nodes.size()	   ) & 0xFF);
	isp->segments[1] = static_cast<uint8_t>((nodes.size()	 >> 8 ) & 0xFF);
	isp->segments[2] = static_cast<uint8_t>((nodes.size()	 >> 16 ) & 0xFF);
	isp->segments[3] = static_cast<uint8_t>((nodes.size()	 >> 24) & 0xFF);
	printf("segments : %u \n",nodes.size());
}
...
namespace {
    ...
    size_t csv_get_offsize(
                       const char *csv_isp_name,
                       struct isp *isp)
    {
        size_t offsize = 0;
        while (offsize < isp->len) {
            if (strcmp(isp->name + offsize, csv_isp_name) == 0) {
                return offsize;
            }else{
                offsize += std::strlen(isp->name+offsize) + 1;
            }
        }
        // not hit
        offsize = isp->len + 1;
		isp->len = offsize + std::strlen(csv_isp_name);
		if(isp->len+1 > MAXBUFF){
			return -1;
		}
			
        std::strcat(isp->name+offsize, csv_isp_name);
...
//			CSV_FIELD_MAX_TEXT,
		...
		while (getline(csv_stream, csv_line)) {
            ...
            ispoffsize = csv_get_offsize(csv_fields[CSV_FIELD_ISP_NAME].c_str(),isp);
			if(ispoffsize < 0 ){
				error_at_line(EX_DATAERR, 0, csv_file_name, csv_line_number,
					      "over max buff size");
				break;
			...
			const binary_trie::edge_type leaf = MAXSEGMENTS + ispoffsize;
			...
			if ((minaddr.inet.s_addr = htonl(atoi(csv_fields[CSV_FIELD_MIN_DECIMAL].c_str()))) == 0) {
				error_at_line(EX_DATAERR, 0, csv_file_name, csv_line_number,
					      "Cannot atoi minimum address: %s",
					      csv_fields[CSV_FIELD_MIN_DECIMAL].c_str());
			}
			if ((maxaddr.inet.s_addr = htonl(atoi(csv_fields[CSV_FIELD_MAX_DECIMAL].c_str()))) == 0) {
				error_at_line(EX_DATAERR, 0, csv_file_name, csv_line_number,
					      "Cannot parse maximum address: %s",
					      csv_fields[CSV_FIELD_MAX_DECIMAL].c_str());
			}
                     ...
                     struct isp *isp)
	{
		if (std::strcmp(csv_file_name, "-") == 0) {
			csv_stream_to_trie(trie, csv_file_name, std::cin, address_family, isp);
		} else {
			std::ifstream csv_stream(csv_file_name, std::ios::in);
			if (!csv_stream) {
				error(EX_NOINPUT, errno, "%s", csv_file_name);
			}
			csv_stream_to_trie(trie, csv_file_name, csv_stream, address_family, isp);
		}
	}
	...
	write_dat_stream(
		...
		struct isp *isp)
	{
		trie.write_binary(dat_stream, isp);
		if (dat_stream.bad()) {
		...
		switch (address_family) {
		case AF_INET: {
			const char structure_info[8] = { 
				static_cast<char>(0xFF), static_cast<char>(0xFF), static_cast<char>(0xFF), static_cast<char>(GEOIP_ISP_EDITION),
				isp->segments[0], isp->segments[1], isp->segments[2], isp->segments[3] };
			dat_stream.write(structure_info, 8);
			break;
		}
		/*
		case AF_INET6: {
			const char structure_info[4] = { static_cast<char>(0xFF), static_cast<char>(0xFF), static_cast<char>(0xFF), 12 };
		...
		struct isp *isp)
	{
		if (std::strcmp(dat_file_name, "-") == 0) {
			write_dat_stream(trie, dat_file_name, std::cout,
					 database_info, address_family, isp);
		} else {
...
main(int argc, char **argv)
{
	...
	binary_trie trie(MAXSEGMENTS);
	csv_file_to_trie(trie, cmdline.csv_file_name, cmdline.address_family, &isp);
